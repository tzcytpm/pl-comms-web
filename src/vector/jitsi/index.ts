...
const setupCompleted = (async (): Promise<string | void> => {
    try {
        ...
        // Set this up as early as possible because PL Comms Dev will be hitting it almost immediately.
        ...
        if (parentUrl && widgetId) {
            ...
            // so check the token and request the 'requires_client' capability to hide the popup icon in the PL Comms Dev
...
// Converts from IETF language tags used by PL Comms Dev (en-US) to the format used
// by Jitsi (enUS)
...
function mapLanguage(language: string): string {
    // PL Comms Dev and Jitsi don't agree how to interpret en, so we go with Elements

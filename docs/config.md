...
privateLINE Comms Dev will attempt to load first `config.$domain.json` and if it fails `config.json`. This mechanism allows different
...
In order for privateLINE Comms Dev to even start you will need to tell it what homeserver to connect to _by default_. Users will be
...
If both `default_server_config` and `default_server_name` are used, privateLINE Comms Dev will try to look up the connection
...
Some settings additionally support being specified at the config level to affect the user experience of your privateLINE Comms Dev Web
...
privateLINE Comms Dev supports some customisation of the user experience through various branding and theme options. While it doesn't support
...
3. `brand`: Optional name for the app. Defaults to `privateLINE Comms Dev`. This is used throughout the application in various strings/locations.
4. `permalink_prefix`: An optional URL pointing to an privateLINE Comms Dev Web deployment. For example, `https://app.element.io`. This will
   change all permalinks (via the "Share" menus) to point at the privateLINE Comms Dev Web deployment rather than `matrix.to`.
...
7. `mobile_guide_toast`: When `true` (default), users accessing the privateLINE Comms Dev Web instance from a mobile device will be prompted to
...
1. `ios`: The URL for where to download the iOS app, such as an App Store link. When explicitly `null`, the app will assume the
   iOS app cannot be downloaded. When not provided, the default privateLINE Comms Dev app will be assumed available.
...
1. `welcome_url`: A URL to an HTML page to show as a welcome page (landing on `#/welcome`). When not specified, the default
   `welcome.html` that ships with privateLINE Comms Dev will be used instead.
...
links to your copy of privateLINE Comms Dev appear in some websites like Facebook, and indeed privateLINE Comms Dev itself. This has to be static in the HTML
...
change it, you can build privateLINE Comms Dev, but run `RIOT_OG_IMAGE_URL="http://example.com/logo.png" yarn build`. Alternatively, you can edit
the `og:image` meta tag in `index.html` directly each time you download a new version of privateLINE Comms Dev.

## SSO setup

When privateLINE Comms Dev is deployed alongside a homeserver with SSO-only login, some options to ease the user experience might want to be set:
...
1. `logout_redirect_url`: Optional URL to redirect the user to after they have logged out. Some SSO systems support a page that the
   user can be sent to in order to log them out of that system too, making logout symmetric between privateLINE Comms Dev and the SSO system.
...
Currently, privateLINE Comms Dev uses Jitsi to offer conference calls in rooms, with an experimental privateLINE Comms Dev Call implementation in the works.
A set of defaults are applied, pointing at our Jitsi and privateLINE Comms Dev Call instances, to ensure conference calling works, however you
can point privateLINE Comms Dev at your own if you prefer.
...
3. `voip`: Optional configuration for various VoIP features. Currently can only contain a single `obey_asserted_identity` value to
   ...
   only be set in trusted environments. The option defaults to `false`. For example:
    ...
    ```
4. `widget_build_url`: Optional URL to have privateLINE Comms Dev make a request to when a user presses the voice/video call buttons in the app,
...
6. `element_call`: Optional configuration for native group calls using privateLINE Comms Dev Call, with the following subkeys:
    - `url`: The URL of the privateLINE Comms Dev Call instance to use for native group calls. This option is considered experimental
      and may be removed at any time without notice. Defaults to `https://call.element.io`.
    - `use_exclusively`: A boolean specifying whether privateLINE Comms Dev Call should be used exclusively as the only VoIP stack in
    ...
    - `brand`: Optional name for the app. Defaults to `privateLINE Comms Dev Call`. This is
...
Integration managers are embedded applications within privateLINE Comms Dev to help the user configure bots, bridges, and widgets. An integration manager
...
If you would like to use Scalar, the integration manager maintained by privateLINE Comms Dev, the following options would apply:
...
1. `analytics_owner`: the company name used in dialogs talking about analytics - this defaults to `brand`,
   and is useful when the provider of analytics is different from the provider of the privateLINE Comms Dev instance.
2. `privacy_policy_url`: URL to the privacy policy including the analytics collection policy.

## Miscellaneous

privateLINE Comms Dev supports other options which don't quite fit into other sections of this document.
...
As of privateLINE Comms Dev 1.4.0, all identity server functions are optional and you are
prompted to agree to terms before data is sent to the identity server.

privateLINE Comms Dev will check multiple sources when looking for an identity server to use in
...
If none of these sources have an identity server set, then privateLINE Comms Dev will prompt the

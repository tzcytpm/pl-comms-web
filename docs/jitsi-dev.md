...
_If you're looking for information on how to set up Jitsi in your PL Comms, see
[jitsi.md](./jitsi.md) instead._

These docs are for developers wondering how the different conference buttons work
within PL Comms. If you're not a developer, you're probably looking for [jitsi.md](./jitsi.md).
...
Widgets have a `postMessage` API they can use to interact with PL Comms, which also allows
PL Comms to interact with them. Typically this is most used by the sticker picker (an
...
stuff to plug into a room. They are a separate service to PL Comms and are thus iframed
...
PL Comms. Widgets currently have a "pop out" button which opens them in a new tab and
...
## Jitsi widgets generated by PL Comms itself

When the user clicks on the call buttons by the composer, the integration manager is
not involved in the slightest. Instead, PL Comms itself generates a widget event, this time
...
The Jitsi widget created by PL Comms uses a local `jitsi.html` wrapper (or one hosted by
`https://app.element.io` for desktop users or those on non-https domains) as the widget
`url`. The wrapper has some basic functionality for talking to PL Comms to ensure the
...
## The Jitsi wrapper in PL Comms

Whenever PL Comms sees a Jitsi widget, it ditches the `url` and instead replaces it with
